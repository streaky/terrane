...
fn main() {
    __terrane_run(async move {
        ...
        let client: TcpStream = __terrane_traced(
            __terrane_await({
                    let __terrane_future = {
                        let __terrane_call = tokio::net::TcpStream::connect(
                            chosen.clone(),
                        );
                        async move {
                            match crate::__terrane_dependency_await_unwind(
        ...
        println!(
            ...
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(| | client.nodelay()))
        ...
        let __terrane_completion_0: TerraneCompletion<()> = async {
            let __terrane_try_0: TerraneCompletion<()> = async {
                let occupied: TcpListener = __terrane_traced_completion!(
                    __terrane_await({ let __terrane_future = { let __terrane_call =
                    tokio::net::TcpListener::bind(chosen); async move { match crate
                ...
                println!(
                    "{}",
                    terrane_scalar_support::scalar_text(&__terrane_raised_completion!(match
                    std::panic::catch_unwind(std::panic::AssertUnwindSafe(| | occupied
                    .ttl())) { Ok(Ok(value)) =>
                    Ok(terrane_int_support::Int::from_u128(value as u128)),
            ...
            match __terrane_try_0 {
                ...
                TerraneCompletion::Error(__terrane_error_0) => {
                            ...
                            == TerraneErrorKind::Custom(DescriptorId(0))
                    {
                        ...
                        println!(
                            "{}",
                            terrane_scalar_support::scalar_text(&String::from("occupied"))
                        );
                    }
                    if !__terrane_handled_0 {
