...
fn main() {
    let text: std::sync::Arc<parking_lot::Mutex<String>> = std::sync::Arc::new(
        parking_lot::Mutex::new(String::from("abc")),
    );
    let seen: std::sync::Weak<parking_lot::Mutex<String>> = std::sync::Arc::downgrade(
        &text,
    );
    println!(
        ...
        __terrane_value = __terrane_owner.lock().clone(); __terrane_value })) as i128))
    );
    println!(
    ...
    println!(
        ...
        .clone(); __terrane_value }))), terrane_scalar_support::scalar_text(&
        (String::from("!"))))))
    );
    println!(
        ...
        = __terrane_owner.lock().clone(); __terrane_value })))))
    );
    let encoded: std::sync::Arc<parking_lot::Mutex<Vec<u8>>> = std::sync::Arc::new(
        parking_lot::Mutex::new(
            terrane_string_support::encode(
                &(({
                    let __terrane_value = text.lock().clone();
                    __terrane_value
                })),
    ...
    let decoded: std::sync::Weak<parking_lot::Mutex<Vec<u8>>> = std::sync::Arc::downgrade(
        &encoded,
    );
    println!(
        ...
        __terrane_value = __terrane_owner.lock().clone(); __terrane_value })),
    ...
    let number: std::sync::Arc<parking_lot::Mutex<i8>> = std::sync::Arc::new(
        parking_lot::Mutex::new(7),
    );
    let observed: std::sync::Weak<parking_lot::Mutex<i8>> = std::sync::Arc::downgrade(
        &number,
    );
    println!(
