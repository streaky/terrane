...
fn main() {
    ...
    println!("{}", terrane_scalar_support::scalar_text(&((terrane_string_support::decode(&(({ let __terrane_owner = decoded.upgrade().expect("reference expired"); let __terrane_value = __terrane_owner.lock().clone(); __terrane_value })), terrane_string_support::Encoding::Utf8)).unwrap_or_else(|error| __terrane_uncaught(TerraneError::from(error).at("/scalar-reference-transparency::main (case.trn:12:13)"))))));
