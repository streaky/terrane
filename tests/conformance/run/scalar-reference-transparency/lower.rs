...
fn main() {
    ...
    println!("{}", terrane_scalar_support::scalar_text(&(format!("{}{}", terrane_scalar_support::scalar_text(&(({ let __terrane_owner = seen.upgrade().expect("reference expired"); let __terrane_value = __terrane_owner.lock().clone(); __terrane_value }))), terrane_scalar_support::scalar_text(&(String::from("!")))))));
    println!("{}", terrane_scalar_support::scalar_text(&(vec![terrane_scalar_support::scalar_text(&(String::from("x"))), terrane_scalar_support::scalar_text(&(String::from("y")))].join(&(({ let __terrane_owner = seen.upgrade().expect("reference expired"); let __terrane_value = __terrane_owner.lock().clone(); __terrane_value }))))));
    let encoded: std::sync::Arc<parking_lot::Mutex<Vec<u8>>> = std::sync::Arc::new(parking_lot::Mutex::new(terrane_string_support::encode(&(({ let __terrane_value = text.lock().clone(); __terrane_value })), terrane_string_support::Encoding::Utf8)));
    let decoded: std::sync::Weak<parking_lot::Mutex<Vec<u8>>> = std::sync::Arc::downgrade(&encoded);
    println!("{}", terrane_scalar_support::scalar_text(&((terrane_string_support::decode(&(({ let __terrane_owner = decoded.upgrade().expect("reference expired"); let __terrane_value = __terrane_owner.lock().clone(); __terrane_value })), terrane_string_support::Encoding::Utf8)).unwrap_or_else(|error| __terrane_uncaught(TerraneError::from(error).at("/scalar-reference-transparency::main (case.trn:14:13)"))))));
    ...
    println!("{}", terrane_scalar_support::scalar_text(&(terrane_int_support::Int::from((({ let __terrane_owner = observed.upgrade().expect("reference expired"); let __terrane_value = __terrane_owner.lock().clone(); __terrane_value })) as i128))));
    println!("{}", terrane_scalar_support::scalar_text(&((terrane_int_support::fixed_addition({ let __terrane_owner = observed.upgrade().expect("reference expired"); let __terrane_value = __terrane_owner.lock().clone(); __terrane_value }, 2)).unwrap_or_else(|error| __terrane_uncaught(TerraneError::from(error).at("/scalar-reference-transparency::main (case.trn:19:13)"))))));
    println!("{}", terrane_scalar_support::scalar_text(&((terrane_int_support::fixed_addition({ let __terrane_owner = observed.upgrade().expect("reference expired"); let __terrane_value = __terrane_owner.lock().clone(); __terrane_value }, 3)).unwrap_or_else(|error| __terrane_uncaught(TerraneError::from(error).at("/scalar-reference-transparency::main (case.trn:20:12)"))))));
    println!("{}", terrane_scalar_support::scalar_text(&(-({ let __terrane_owner = observed.upgrade().expect("reference expired"); let __terrane_value = __terrane_owner.lock().clone(); __terrane_value }))));
    ...
    println!("{}", terrane_scalar_support::scalar_text(&((terrane_int_support::fixed_multiplication({ let __terrane_value = owner.lock().clone(); __terrane_value }, 2)).unwrap_or_else(|error| __terrane_uncaught(TerraneError::from(error).at("/scalar-reference-transparency::main (case.trn:24:12)"))))));
}
