...
// Namespace: app
fn main() {
    let point: Point<f32> = __terrane_raised(
...
pub use terrane_point_witness::Point;
pub fn terrane_static_trn_506f696e74_new(
    ...
    match std::panic::catch_unwind(
        std::panic::AssertUnwindSafe(|| terrane_point_witness::Point::<f32>::new(x, y)),
    ) {
        Ok(value) => Ok(value),
        Err(payload) => {
            Err(
                crate::__terrane_dependency_panic(
                    payload,
                    "terrane-point-witness",
                    "terrane_point_witness::Point<f32>",
                ),
            )
        }
    }
