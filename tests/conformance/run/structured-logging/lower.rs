...
// Runtime support: async_native.rs, executor_parallel.rs, channels.rs, tasks_native_parallel.rs, time_inactive.rs, platform_data_base.rs, platform_documents.rs, logging_values.rs, platform_capability_types.rs, platform_result_type.rs, platform_int_conversion.rs, platform_capability_base.rs, platform_concurrency.rs
    ...
    while open {
        ...
        if event.is_some() {
            let written: LogOutcome = write_event(
                ...
                event.as_ref().expect("semantic optional narrowing").clone().clone(),
            );
            if written.failed {
                succeeded = false;
                open = false;
