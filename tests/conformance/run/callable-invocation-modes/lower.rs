    ...
    __terrane_run(async move {
                ...
                Box::pin(async move {
                    {
                        ...
                        *async_counter
                            .lock()
                            .expect("mutable async callable state lock poisoned") = callable_capture_value;
                    }
        ...
        > = {
            ...
            | -> std::pin::Pin<Box<dyn Future<Output = _> + Send>> {
                ...
                Box::pin(async move {
                    {
                        let mut receiver = receiver_state
                            .lock()
                            .expect("mutable async callable state lock poisoned")
                            .clone();
                        let result = receiver.add(argument_0).await;
                        *receiver_state
                            .lock()
                            .expect("mutable async callable state lock poisoned") = receiver;
                        result
                    }
                })
            })
                ...
                Box::pin(async move {
                    {
                        let mut receiver = receiver_state
                            ...
                            .clone();
                        let result = receiver
                            .add(argument_0, argument_1, argument_2)
                            .await;
                        *receiver_state
                            .lock()
                            .expect("mutable async callable state lock poisoned") = receiver;
