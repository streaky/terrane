...
fn main() {
    let values: std::sync::Arc<parking_lot::Mutex<terrane_collection_support::List<terrane_int_support::Int>>> = std::sync::Arc::new(parking_lot::Mutex::new(terrane_collection_support::List::<terrane_int_support::Int>::new(vec![terrane_int_support::Int::from(1_i128)])));
    let owner: std::sync::Arc<parking_lot::Mutex<terrane_collection_support::List<terrane_int_support::Int>>> = (values).clone();
    let observer: std::sync::Weak<parking_lot::Mutex<terrane_collection_support::List<terrane_int_support::Int>>> = std::sync::Arc::downgrade(&values);
    (owner.lock()).append(terrane_int_support::Int::from(2_i128));
    println!("{}{}", terrane_scalar_support::scalar_text(&(terrane_int_support::Int::from((({ let __terrane_value = values.lock().clone(); __terrane_value })).length()))), terrane_scalar_support::scalar_text(&(((({ let __terrane_owner = observer.upgrade().expect("reference expired"); let __terrane_value = __terrane_owner.lock().clone(); __terrane_value })).get_or_error((terrane_collection_support::index_from_int(&(terrane_int_support::Int::from(1_i128)))).unwrap_or_else(|error| __terrane_uncaught(TerraneError::from(error).at("/references-provenance::main (case.trn:10:25)"))))).unwrap_or_else(|error| __terrane_uncaught(TerraneError::from(error).at("/references-provenance::main (case.trn:10:25)"))))));
    ...
    println!("{}", terrane_scalar_support::scalar_text(&(((transferred).get_or_error((terrane_collection_support::index_from_int(&(terrane_int_support::Int::from(0_i128)))).unwrap_or_else(|error| __terrane_uncaught(TerraneError::from(error).at("/references-provenance::main (case.trn:13:10)"))))).unwrap_or_else(|error| __terrane_uncaught(TerraneError::from(error).at("/references-provenance::main (case.trn:13:10)"))))));
}
