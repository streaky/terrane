...
mod __terrane_trace {
    ...
    pub static SITES: [Site; 4] = [
        ...
        { Site { function: 0, file: 0, line: 12, column: 25, end_line: 12, end_column: 47 } },
        /* terrane-site-row: site 2: /app::main (src/main.trn:12:74-12:93) */
...
// Namespace: deps/terrane-borrowed-struct-witness
...
impl Descriptor {
    pub fn terrane_construct(
        native_label: String,
        values: terrane_collection_support::List<String>,
        offset: u32,
        note: impl Into<Option<String>>,
        extra: impl Into<Option<terrane_collection_support::List<String>>>,
    ) -> Self {
        Self {
            ...
            offset: offset,
            note: note.into(),
            extra: extra.into().map(|value| value.into_vec()),
        }
    }
    fn terrane_native_view(&self) -> terrane_borrowed_struct_witness::Descriptor<'_> {
        terrane_borrowed_struct_witness::Descriptor {
...
pub use terrane_borrowed_struct_witness::Owned;
pub fn consume(descriptor: Descriptor) -> Result<String, crate::TerraneForeignError> {
    match std::panic::catch_unwind(
        std::panic::AssertUnwindSafe(|| terrane_borrowed_struct_witness::consume(
            descriptor.terrane_native_view(),
        )),
    ) {
        Ok(value) => Ok(value),
        Err(payload) => {
            Err(
                crate::__terrane_dependency_panic(
                    payload,
                    "terrane-borrowed-struct-witness",
                    "terrane_borrowed_struct_witness::consume",
                ),
            )
        }
    }
}
pub fn summarize(descriptor: &Descriptor) -> Result<String, crate::TerraneForeignError> {
    match std::panic::catch_unwind(
        std::panic::AssertUnwindSafe(|| terrane_borrowed_struct_witness::summarize(
            &descriptor.terrane_native_view(),
        )),
    ) {
        Ok(value) => Ok(value),
        Err(payload) => {
            Err(
                crate::__terrane_dependency_panic(
                    payload,
                    "terrane-borrowed-struct-witness",
                    "terrane_borrowed_struct_witness::summarize",
                ),
            )
        }
    }
}
pub fn summarize_owned(value: &Owned) -> Result<String, crate::TerraneForeignError> {
    match std::panic::catch_unwind(
        std::panic::AssertUnwindSafe(|| terrane_borrowed_struct_witness::summarize_owned(
            value,
        )),
    ) {
        Ok(value) => Ok(value),
        Err(payload) => {
            Err(
                crate::__terrane_dependency_panic(
                    payload,
                    "terrane-borrowed-struct-witness",
                    "terrane_borrowed_struct_witness::summarize_owned",
                ),
            )
        }
    }
