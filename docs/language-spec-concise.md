...
artifact = "executable"   # optional; executable | dynamic-library
...
- Authored manifest filename: `package.toml`; syntax is TOML; unknown fields rejected. Optional `artifact` is `executable` (default) or `dynamic-library`; the latter emits a Cargo `cdylib`, while `run`/`debug` require an executable package and reject it. Terrane declarations do not yet export host-visible symbols automatically; a maintained authored Rust module supplies the host entrypoint and may call lowered Terrane functions within the generated crate.
