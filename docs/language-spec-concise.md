...
projection_inventory: ordinary package analysis refreshes the ignored terrane-projection.generated.trn beside package.toml; it comments every unavailable native declaration with exact path, reason, and current demand sites, ordered as currently required entries, a required-admitted index, a visible separator, then unreachable unavailable entries, followed by the complete generated Terrane source; demanded unavailable functions include deterministically recoverable function shapes, generic constraints, callback parameter contracts, and callback method signatures, while demanded modules are identified as namespace containers; native declarations referenced by a required partial contract inherit required status recursively by stable Rustdoc identity and map back through public aliases, with each inherited requirement naming its requiring contract; cycles terminate by identity; partial contracts preserve known native structure without becoming callable or weakening diagnostics; unreachable unavailable entries are non-failing evidence; only a source demand for one fails semantic analysis, and the diagnostic links to the inventory
