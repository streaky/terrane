...
projection_interfaces: eligible fixed-signature Rust traits => nominal Terrane interfaces; &self/&mut self/self => shared/mutable/consuming requirements; required members demand class implementations, provided members supply overridable defaults; local classes adopt projected identities with implements and lower to foreign impls; immediate concrete generic or impl-Trait inputs specialize from the written class argument; static methods, generic methods, unresolved associated types, higher-ranked lifetimes, and unprojectable member types decline with stable artifact reasons; Rust traits project to Terrane interfaces, never Terrane traits, because the latter are source implementation composition through uses
