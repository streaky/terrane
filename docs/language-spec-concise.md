...
applied_type_annotations: constructor arguments are recursively resolved type expressions, so a closed projected interface may appear inside lists, tuples, maps, channels and other aggregates; `|` belongs to the current `of` argument, while grouping makes the completed application optional, as in `(sequence of int)|none`
