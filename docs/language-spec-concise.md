...
projection_inventory: ordinary package analysis refreshes the ignored terrane-projection.generated.trn beside package.toml; it comments every unavailable native declaration with exact path, reason, and current demand sites, ordered as currently required entries, a visible separator, then unused entries, followed by the complete generated Terrane source; demanded unavailable functions include deterministically recoverable function shapes, generic constraints, callback parameter contracts, and callback method signatures, while demanded modules are identified as namespace containers; partial contracts preserve known native structure without becoming callable or weakening diagnostics; undemanded unavailable entries are non-failing evidence; only a source demand for one fails semantic analysis, and the diagnostic links to the inventory
