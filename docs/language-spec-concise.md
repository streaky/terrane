...
order: explicit unit -> integration -> end-to-end tier order, then logical path, source order, and function name; substring/exact/glob/regex filters are mutually exclusive; semantic discovery covers every populated root, while only tiers containing selected cases are lowered and built
...
context: stable identity/tier/seed/temporary-directory, typed deadline duration, repeatable controlled process arguments, checked explicit monotonic-time advancement, and test-clock sleeps that advance immediately to their target when awaited
