...
- Cancellation is cooperative at defined points, but observation while suspended promptly drops the in-flight operation. Compiler-separated `finally` state then runs exactly once, innermost first, and async cleanup is driven shielded from the initiating request before join completes. Owned foreign operation state runs Rust `Drop`; cleanup-owned state survives until cleanup. Reject overlapping exclusive ownership, insufficient-lifetime borrows, or other captures that cannot split operation and cleanup state.
