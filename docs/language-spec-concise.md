...
projection_artifacts: optional TERRANE_PROJECTION_ARTIFACT_URL trusted HTTPS lookup before local rustdoc; exact dependency/features/defaults/target/build pin/rustdoc pin+format/projection-schema match only; any miss/mismatch falls back locally; verified result enters ordinary offline cache and records source=Remote
