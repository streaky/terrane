...
- Authored manifest filename: `package.toml`; syntax is TOML; unknown fields rejected. Optional `artifact` is `executable` (default) or `dynamic-library`; the latter emits a Cargo `cdylib`, while `run`/`debug` require an executable package and reject it.
