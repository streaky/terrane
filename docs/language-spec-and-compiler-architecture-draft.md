...
A core facility backed by Rust follows the same boundary contract as an authored dependency wrapper:
the source-visible declarations and policy remain in Terrane, while a deliberately narrow private
host binding crosses into the audited support crate. Bootstrap ordering means compiler-shipped core
packages use compiler-private bindings rather than importing a projected Rust package, but those
bindings are not source-visible, do not create public core objects, and remain capability-gated.
...
monotonic/wall-clock observation and wake-driven timer registration. The deliberately tiny
`terrane-signal-support` crate is the signal-specific unsafe layer: it installs only actively
subscribed host handlers, counts observations with signal-safe atomics, wakes the broker through a
self-pipe, and restores captured dispositions. The safe platform-support broker owns a joined worker
thread, fan-out, coalescing, ordering, overflow reporting, and subscription lifecycle. No signal
handler executes Terrane code, allocates, locks, or calls the executor.
...
Each Terrane version selects one stable Rust release. The compiler workspace pins that release in
`rust-toolchain.toml` and declares it as its `rust-version`; `BUILD_TOOLCHAIN` and generated crates
repeat the same release in both `rust-version` and a generated `rust-toolchain.toml`. This pin is
the default for `check`, `build`, and `run`, makes ambient toolchain updates irrelevant to unchanged
Terrane projects, and remains in the generated directory for direct Cargo debugging. A package may
explicitly set `rust-toolchain = "system"` to use its invoking environment instead; the build
metadata records that escape hatch, and the selected compiler must still meet Terrane's declared
version.
...
test-root discovery, one typed registry per populated tier, native process isolation/capture, and
the same source-to-Rust build pipeline used by ordinary commands.
...
Unit tests compile with the production source set and receive only the namespace-private access
their declared namespace ordinarily permits. Integration tests compile independently as external
consumers of the package's public surface. End-to-end tests compile independently and drive the
built application artifact through an explicit process fixture from capability-gated
