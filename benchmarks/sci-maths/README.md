...
`lower` refreshes each lane's declared inspectable lowering by passing the resolved `lower-output`
path to its lowering command. The Terrane and Terrane+numr lanes write clean application lowering
to `main.lowered.rs` and compiler infrastructure to `main.lowered.support.rs` beside every
`main.trn`, keeping both generated Rust receipts with the solution.
...
`$implementation_dir`, `$lowered` for a declared lowering destination, and, after preparation,
`$prepared`. Use `${name}` where a placeholder touches adjacent text. Lowering commands must write
their declared `$lowered` file; the runner does not reinterpret stdout as generated source.
