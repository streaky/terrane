...
The `scientific-stack` group currently contains pairwise oscillatory Bessel-kernel energy and gamma survival-model calibration. Python uses vectorized NumPy arrays and SciPy special functions. Terrane+numr imports `numr` 0.7.0 through `/deps/numr/algorithm/special` and calls its public scalar special-function entry points. Pure Terrane implements the same special-function algorithms from scalar arithmetic and the compiler's foundational floating-point properties: a rational Bessel approximation uses `square-root`, `sine`, and `cosine`, while incomplete-gamma series and continued fractions use `natural-log` and `exponential` around a source-authored log-gamma approximation. All three lanes receive the same size, generate the same data in-process, evaluate every specified term, and return the same scalar contract. The suite does not substitute a hand-tuned foreign implementation for pure Terrane.
